...
prompt_nix() {
  if [[ -n $IN_NIX_SHELL ]]; then
    prompt_segment blue black "\uf313 $NIX_SHELL_PACKAGES"
    return
  fi

  if [[ "$PATH" == *"/nix/store"* ]]; then
    # Extract the nix store path containing the package
    nix_store_path=$(echo $PATH | tr ':' '\n' | grep '/nix/store' | head -n 1)
    if [[ -n $nix_store_path ]]; then
      # Get the parent directory of the bin folder to extract the package name
      nix_package=$(basename $(dirname $nix_store_path))
      prompt_segment blue black "\uf313 $nix_package"
    fi
  fi
}
